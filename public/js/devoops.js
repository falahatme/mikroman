...
function bitProc(number){

    var returnString = "";

    if(number > 1048576){
         number = (number/1048576).toFixed(2);
        returnString = number + " Gb";
    }else if(number > 1024){
         number = (number/1024).toFixed(2);
        returnString = number + " Mb";
    }else{
        returnString = number + " Kb";
    }
    return returnString;
...
//
// Helper for draw Google Chart By String
//
function drawGoogleChartByString(chart_data, chart_options, element, chart_type) {
	// Function for visualize Google Chart
var chart_type = eval("google.visualization."+chart_type);
drawGoogleChart(chart_data, chart_options, element, chart_type);
}
//
...
function MorrisChart1(){
	var day_data = [
		{"period": "2013-10-01", "licensed": 3407, "sorned": 660, "sum" : 10000},
		{"period": "2013-09-30", "licensed": 3351, "sorned": 629, "sum" : 10000},
		{"period": "2013-09-29", "licensed": 3269, "sorned": 618, "sum" : 10000},
		{"period": "2013-09-20", "licensed": 3246, "sorned": 661, "sum" : 10000},
		{"period": "2013-09-19", "licensed": 3257, "sorned": 667, "sum" : 10000},
		{"period": "2013-09-18", "licensed": 3248, "sorned": 627, "sum" : 10000},
		{"period": "2013-09-17", "licensed": 3171, "sorned": 660, "sum" : 10000},
		{"period": "2013-09-16", "licensed": 3171, "sorned": 676, "sum" : 10000},
		{"period": "2013-09-15", "licensed": 3201, "sorned": 656, "sum" : 10000},
		{"period": "2013-09-10", "licensed": 3215, "sorned": 622, "sum" : 10000}
	];
	Morris.Bar({
     ...
     barColors: ['#0979E8', '#35D442', '#408080'],
		ykeys: ['licensed', 'sorned', 'sum'],
		labels: ['Licensed', 'SORN', 'MAX'],
		xLabelAngle: 60
	});
}
//
...
function CoinDeskGraph(){
	...
	$.getJSON(jsonURL, function(result){
		...
		$.each(result.bpi, function(key, val){
			...
			var newDate=parseDate[0]+"-"+parseDate[1]+"-"+parseDate[2];
...
function TestTable3(){
    ...
    var header_inputs = $("#datatable-3 thead input");
    header_inputs.on('keyup', function(){
        /* Filter on the column (the index) of this element */
        oTable.fnFilter( this.value, header_inputs.index(this) );
    })
...
function SparklineLoop(){
	...
	//SparkLineDrawBarGraph($('#sparkline-3'), sparkline_arr_3.map(SmallChangeVal), '#fff');
}
//
