                ...
                foreach($router_ips as $id => $router_ip){
                                            ...
                                            foreach($router_devices[$id]['interfaces'] as $interfaceKey => $interfaceName){
                                                ...
                                                if(@($router_devices[$id]['newbytesIn'][$interfaceKey] - $router_devices[$id]['bytesIn'][$interfaceKey]) > 0){
                                                        $speeds[$interfaceKey]['rx'] = ($router_devices[$id]['newbytesIn'][$interfaceKey] - $router_devices[$id]['bytesIn'][$interfaceKey]) /  128 / ($router_devices[$id]['newsectime'] - $router_devices[$id]['sectime']);
                                                ...
                                                if(@($router_devices[$id]['newbytesOut'][$interfaceKey] - $router_devices[$id]['bytesOut'][$interfaceKey]) > 0){
                                                        $speeds[$interfaceKey]['tx'] = ($router_devices[$id]['newbytesOut'][$interfaceKey] - $router_devices[$id]['bytesOut'][$interfaceKey]) /  128 / ($router_devices[$id]['newsectime'] - $router_devices[$id]['sectime']);
                                        ...
                                        foreach($router_devices[$id]['interfaces'] as $key=>$value){
                                                ...
                                                $active = table('snmpinterfaces')->where(array('router_id='.$id, 'interface_id='.$key, '`active`=1'))->run();
                                                if(@$active->num_rows==1){
                                                        ...
                                                        table('snmplog')->fields($fields)->insert();
